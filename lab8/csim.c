...
//检查cache中相应的set有无该tag，并视情况更新该set，返回状态
//返回1表示set中有该tag，hit了。
//返回2表示set中无该tag，但该set未满，miss。
//返回3表示set中无该tag，且该set已满，miss and eviction。
...
//解析命令，将地址转化为二进制01字符串，从而提取set，tag
//以此调用searchAndSolve，返回结果。
